...
## Status reconciliation (2026-08-31)

The sprint headers below said COMPLETE while no checkbox was ever ticked
(`openspec list` read 0/38 against a shipped v0.0.1). Reconciled against
repo evidence only: an item is ticked when its deliverable artifact exists
here (script/test/dump/doc) or its completion is recorded in a closed
queue (release-v001-dynamic T1–T8) or a raw dump under `benchmarks/raw/`.
Boxes NOT ticked are genuinely open: **3.7** (greedy 10/10 box re-run —
queued in `harden-quality-gates` G3; distribution-level result documented
in the README), **4.6** (HF publish — deferred by user, staged in
`huggingface/`), **4.7** (community posting — drafts done in
`notes/community-drafts.md`, posting left to the user). Two divergences
kept honest inline: 2.1's preflight shipped **inlined** in the boot
harnesses (k1/w3/gpu_ab), not as `solo_preflight.sh`; 5.4's "fix" was
**not applied because 5.3 refuted the mismatch** (capture semantics are
the unweighted mean on both sides — retracted finding, research/07 risk 7
+ `test_hc_collapse.py` header).
...
- [x] **1.1 Stage GLM-5.3-Flash target HF files.** The draft checkpoint has no tokenizer; `DFlashModel.set_vocab` (qwen.py:647-673) delegates to the target arch's vocab handler and needs real files, not the GGUF shards. Run:
...
- [x] **2.1 Solo-run preflight.** Write `scripts/solo_preflight.sh`: (a) verify :8086 not serving (`ss -ltn | grep :8086` empty, or `systemctl is-active llama-server-qwen-flash` ≠ active), (b) `free -g` shows ≥ 165 GB available (147 GB target + ~5 GB draft + headroom; box has 251 GB), (c) `ss -ltn | grep :8100` empty. Gate: all three pass. If production must be stopped: coordinate a window, `sudo systemctl stop llama-server-qwen-flash`, and restart it in 2.6. **STOP GATE**: never start the 147 GB load while :8086 is live.

- [x] **2.2 Write the systemd unit.** Create `systemd/llama-server-glm5-dflash2.service` in this repo, install to `/etc/systemd/system/`. Copy the flag set from `/etc/systemd/system/llama-server-glm5.service` (port 8100, IQ4_XS 5-shard target, `--threads -1 --numa distribute --load-mode mlock`, 128k ctx, FA on, f16 KV) and add:
  ```
...
- [x] **5.1 Overhead calibration (no acceptance noise).** Solo run with
...
- [x] **5.2 Config sweep (fast levers).** Solo windows, one change at a time,
...
- [x] **5.3 mHC extraction probe (the big fix, risk 4).** Build the Sprint 3.2
...
- [x] **5.4 mHC fix.** If 5.3 confirms mismatch: add the gated contraction to
...
- [x] **5.5 Re-validate losslessness.** After config + mHC changes: re-run the
  greedy spec-on == spec-off check (3.7). Gate: 10/10 identical outputs.

- [x] **5.6 Sprint 4 benchmark with final config.** Re-run 4.2/4.3 arms with
